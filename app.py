...
with tab1:
    ...
    # Line + Marker Plot (Dot plot with rigid connecting lines)
    df = data_store.get_logs_df()
    if not df.empty:
        df['effective_miles'] = df.apply(
            lambda r: agent.calculate_effective_miles(r['distance_miles'], r.get('cross_train_mins', 0)), axis=1)

        # Group by daily log date for a clean time series line chart
        fig = px.line(
            df,
            x='log_date',
            y='effective_miles',
            markers=True,
            title="Daily Training Volume (Run + Cross-Train Equivalent)",
            labels={'log_date': 'Date', 'effective_miles': 'Effective Mileage'},
...
with tab2:
    ...
    with st.form("log_form"):
        ...
        with col2:
            ...
            pain = st.selectbox("Specific Joint/Muscle Pain",
                                ["None", "Knee Patellar", "IT Band", "Shin Splints", "Achilles Plantar"])
